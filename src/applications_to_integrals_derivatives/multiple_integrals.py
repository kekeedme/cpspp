...
def polarforce(theta,phi):
    ...
    deno =(1/(np.cos(theta)**2 * np.cos(phi)**2)) * (1/(np.tan(theta)**2 + np.tan(phi)**2 + 1)**(3/2))
    num = _G * SIGMA * MASS
    return num * deno
...
force_list.append(force) #calculate the first point using integral on infinite range since z =0
